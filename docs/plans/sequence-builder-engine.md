...
**Real `SEND_CONNECTION_REQUEST`/`WITHDRAW_CONNECTION` retest (2026-07-07) — one real bug found and fixed, verification still blocked by the account itself, not the code.**

Retested against a real, user-provided profile (`christopher-njokanma-440198a8`) once the test account had a fresh session. Found and fixed a real bug in `packages/browser/src/actions/sendConnect.ts`: LinkedIn doesn't consistently render "Connect" as a direct top-level button — on some profiles (confirmed by the user via screenshot) it's tucked inside the "More" overflow menu instead, varying per-profile rather than per-account. `sendConnect` now checks for the direct button first and falls back to opening "More" and clicking "Connect" from the revealed dropdown. Also added the same dead-session/authwall URL check `scrapeSearch.ts` already had, so an expired session surfaces as a clear "re-import fresh cookies" error instead of a confusing "Connect button not found" timeout — this exact confusion is what made the underlying session death hard to diagnose the first time.

**However, verification itself did not complete**: across two more attempts (each requiring a fresh cookie re-import), the account's session died again almost immediately after a `SEND_CONNECTION_REQUEST` attempt specifically — while passive session-health checks (feed visits) had been staying healthy for hours in between. This pattern (healthy for passive browsing, dies specifically around connect actions) suggests LinkedIn may be applying heightened scrutiny to this specific account's connection-request behavior after all the automated testing it's been through — not a remaining code bug. The user's own manual browsing during this session (screenshots) confirmed they were logged in and using LinkedIn normally around the same time, so this isn't simply "the account is dead" in general.

Per the user's explicit call after being warned about this pattern ("try once now, stop and reassess if it fails again for a real reason") — stopped after the one retry rather than continuing to retry against a possibly-flagged account. `VISIT_PROFILE`/`LIKE_POST` remain the only step types confirmed working via a real send; `SEND_CONNECTION_REQUEST`/`WITHDRAW_CONNECTION` are implemented identically to those (same processor pattern, same guard chain, same browser-worker lifecycle) and now free of the two bugs found today, but still haven't completed a real end-to-end send+withdraw cycle. No dangling real connection request was left behind on any attempt — every failure happened before or without ever reaching LinkedIn's actual send action.

**If revisiting this**: don't retry on this same account without a clear reason to believe the scrutiny has cooled off, or without accepting the risk explicitly. A cleaner path might be a different/less-tested account, or accepting code-review-level confidence (same pattern as the two already-proven action types) in place of a live send on this specific account.
