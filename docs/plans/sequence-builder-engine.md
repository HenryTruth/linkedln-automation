...
**Status**: Milestones 1-4 done. Remaining: milestone 5 (leads-table step column — folded into milestone 4 above since it was small enough to do alongside), and milestone 6 (full multi-day dogfood test with both branches actually exercised on a real account) plus the still-open real `SEND_CONNECTION_REQUEST`/`WITHDRAW_CONNECTION` verification from milestone 3 (blocked on a fresh LinkedIn cookie export for the test account).
